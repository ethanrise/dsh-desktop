...
Normal will-quit, intentional GPU relaunch, and in-app update install clear the session marker. A leftover session marker is **not** reported as unclean-exit: overwrite installs, forced termination, and first-launch logs that never reached readiness used to look like crashes and opened the consent dialog. Already-queued unclean-exit reports are discarded on flush without asking or uploading. Proven failures (startup-failure, harness-crash, renderer-crash, gpu-crash, main-crash) still require consent. If the app never reaches initialization again, there is no guarantee of delivery. Initialization/reporting failure does not prevent application startup.
